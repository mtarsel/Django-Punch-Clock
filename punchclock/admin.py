...
class TimecardAdmin(admin.ModelAdmin):
	def get_urls(self):
		urls = super(TimecardAdmin, self).get_urls()
		my_urls = patterns('',
			(url(r'^admin/punchclock/timecard$', self.admin_site.admin_view(self.timecards, cacheable=True)))
		)
		return my_urls + urls
		
	def timecards(Timecard, request, queryset):
		queryset.update(status='g')
		#for obj in queryset:
		#	do_something_with(obj)
		make_published.short_description = "Mark selected students for Timecards"
...
class ReportAdmin(admin.ModelAdmin):
	def get_urls(self):
		...
		my_urls = patterns('',
			(url(r'^admin/punchclock/reports$', self.admin_site.admin_view(self.reports, cacheable=True)))
		)
		return my_urls + urls
